...
public class MainActivity2 extends AppCompatActivity {
    ...
    private Question randomQuestions[] = new  Question[10];
    private String questionsSitutation = new String();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        questionNumberTextView.setText("Test"+ String.valueOf((getIntent().getExtras().getInt("testNumber")))+"\n1/"+randomQuestions.length);
        View.OnClickListener onClickListener = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (v.getId() == correctAnswerButton.getId()){
                    ...
                    questionsSitutation = questionsSitutation.concat("\nQuestion "+ (questionCounter+1) +": correct");
                }else{
                    questionsSitutation = questionsSitutation.concat("\nQuestion "+ (questionCounter+1) +": wrong");
                    v.setBackgroundColor(Color.RED);
                    findViewById(correctAnswerButton.getId()).setBackgroundColor(Color.GREEN);
                }
                ...
                hndler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if(questionCounter==randomQuestions.length){
                            ...
                            extras.putString("result", usernameTextView.getText().toString()+"\nScore: "+ score + "/100\nCorrect Answer: "+ (int)(((double)score/100)*randomQuestions.length) + "\nWrong Answer: "+ (int)(((double)(100-score)/100)*randomQuestions.length)+ questionsSitutation);
                            intent.putExtras(extras);
                            startActivity(intent);
                        }else {
                            updateQuestion(randomQuestions [questionCounter], questionImage, questionText, new Button[]{answerButton1, answerButton2, answerButton3, answerButton4, answerButton5});
                            questionNumberTextView.setText("Test"+ String.valueOf(getIntent().getExtras().getInt("testNumber"))+"\n"+(questionCounter + 1)+"/"+randomQuestions.length);
                        }
                    }
                }, 1000);
            }
    ...
    void mixAndChooseQuestions(Question questions[], Question randomQuestions[]){
        ...
        for (int i = 0; i<randomQuestions.length; i++){
            if (getIntent().getExtras().getInt("testNumber") == 2){
                randomQuestions[i]=questionsTemp.get(i+10);
            }else{
